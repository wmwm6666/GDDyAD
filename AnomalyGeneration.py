...
def anomaly_generation(ini_graph_percent, anomaly_percent, data, n, m, seed = 1):
    ...
    # clustering nodes to clusters using spectral clustering 聚类是为了不在相似节点之间生成异常边
    kk = 42 #3#10#42#42
    ...
    fake_edges = processEdges(fake_edges, data) # 将负采样节点进行去重和去除等操作
    ...
    train_mat = train_mat + train_mat.transpose() # 因为之前是单向的边，所以要加上转置使矩阵对称

    return synthetic_test, train_mat, train, test  # 注入异常边的测试集 ，训练集的邻接矩阵，训练集边
...
def processEdges(fake_edges, data):
    ...
    fake_edges[idx_fake] = tmp # 以上依然是将起始节点序号调整为小于目标节点
...
def edgeList2Adj(data):
    ...
    :param data: edge list【n*2】
    :return: adjacency matrix which is symmetric【n*n】
    """
